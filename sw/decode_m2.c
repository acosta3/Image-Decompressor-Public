	...
	for (color=0; color<3; color++) {
		...
		for (i=0; i<height; i+=8) for (j=0; j<width_temp; j+=8) {	//i*width_temp+j+offset_read is the read address of the top left corner of the current 8x8 block
			...
			for (k=0; k<8; k++) for (m=0; m<8; m++) {
				...
				//if(color == 0) printf("%d\n", j);
				if(color == 2 && i == 0 && j== 8) printf("FETCH - [%d, %d] - decimal: %d - hex:%x\n", k, m, temp_matrix1[k][m], temp_matrix1[k][m]);
			...
			for (k=0; k<8; k++) for (m=0; m<8; m++) {
				...
				if(color == 2 && i == 0 && j ==8) printf("T - [%d, %d] - decimal: %d - hex:%x\n", k, m, temp_matrix2[k][m], temp_matrix2[k][m]);
			...
			for (k=0; k<8; k++) for (m=0; m<8; m++) {
				...
				if(color == 2 && i == 0 && j == 8) printf("S - [%d, %d] - decimal: %d - hex:%x\n", k, m, temp_matrix1[k][m], temp_matrix1[k][m]);
			}
			...
			for (k=0; k<8; k++) for (m=0; m<8; m+=2) {	//first piece of data goes in high byte
				...
				if(color == 2 && i == 0 && j == 8) printf("FINAL PRODUCT - [%d, %d] - decimal: %d - hex:%x\n", k, m, sram[ ((i+k)*width_temp + j+m) / 2 + sram_offset_address_write ], sram[ ((i+k)*width_temp + j+m) / 2 + sram_offset_address_write ]);
			}
		}
	}
